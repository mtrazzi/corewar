...
static int		ft_parse_chp(t_env *e, char *file_name, int chp_nb)
{
	...
	if (++(e->par.nb_chp) > MAX_PLAYERS)
		return (ft_error_vm(ERR_NB_CHP, STR_ERR_NB_CHP));
	new_chp.nb = chp_nb;
...
int		ft_check_option(t_env *e, int i, int ac, char **av)
{
	...
	else if (!ft_strcmp(av[i] + 1, "n"))
	{
		if (!ft_is_int(av[i + 1]) || i + 2 >= ac || !ft_is_valid_ext(av[i + 2]))
				return (-1);
		if (ft_parse_chp(e, av[i + 2], ft_atoi(av[i + 1])) < 0)
				return (-1);
		ft_printf("ft_atoi(av[i + 1]) : %d\n", ft_atoi(av[i + 1]));
		return (2);
	}
	return (1);
}
...
int		parse_arg_vm(int ac, char **av, t_env *e)
{
	...
	if (ac < 2)
		return (ft_error_vm(ERR_PARSING_ARG, STR_ERR_USAGE));
	i = 1;
	while (i < ac)
	{
		if (av[i][0] == '-')
		{
			if ((ret = ft_check_option(e, i, ac, av)) < 0)
				return (ft_error_vm(ERR_PARSING_OPT, STR_ERR_USAGE));
			i += ret;
		}
		else if (!ft_is_valid_ext(av[i]) || ft_parse_chp(e, av[i], --chp_nb) < 0)
			return (ft_error_vm(ERR_PARSING_CHP, STR_ERR_USAGE));
		i++;
	}
	return (0);
}
